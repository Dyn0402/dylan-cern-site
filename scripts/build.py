...
# Sources and outputs are UTF-8 everywhere, and every read_text/write_text here
# says so — the platform default is cp1252 on Windows, which decodes an em dash
# into mojibake without complaining and dies on anything above U+00FF. The
# console is the other half of the same problem: warnings below contain em
# dashes, and printing one must not be what fails a build.
...
def prune_notes(keep, check):
    ...
    for f in sorted(out.glob("*.html")):
        if f.name in keep:
            continue
        if not f.read_text(encoding="utf-8").startswith("<!-- Generated by scripts/build.py"):
...
def render_sw(urls):
    ...
    out = SW_TEMPLATE.read_text(encoding="utf-8")
